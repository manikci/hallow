...
#rm -rf /manikci /floppy /virtio /media/* /tmp/*
...
sudo mkdir /manikci /floppy /virtio
...
#sudo wget -P /manikci https://archive.org/download/WS2012R2/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /manikci/WS2012R2.ISO http://download.microsoft.com/download/6/2/A/62A76ABB-9990-4EFC-A4FE-C7D698DAEB96/9600.17050.WINBLUE_REFRESH.140317-1640_X64FRE_SERVER_EVAL_EN-US-IR3_SSS_X64FREE_EN-US_DV9.ISO 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /manikci https://ia601506.us.archive.org/4/items/WS2012R2/WS2012R2.ISO
else
	echo -e "${RED}[Error]${NC} ${YELLOW}Sorry! OS image urls are available , please report about this issue on Github page : ${NC}https://github.com/manikci/Linux-to-Windows-with-QEMU"
	echo "Exiting.."
	sleep 30
	exit 1
fi
...
sudo wget -P /floppy https://downloadmirror.intel.com/23073/eng/wired_proset_26.4_x64.zip # Intel Network Adapter for Windows Server 2012 R2 
...
custom_param_os="/manikci/"$(ls /manikci)
...
echo -e "Now you can access \"VNC viewer\" or \"Remote Desktop Application\" (if your server 'Remote Desktop' is enabled)."
echo "Job Done :)"
fi
else
echo "Job Done :)"
